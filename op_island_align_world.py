...
class op(bpy.types.Operator):
    ...
    axis: bpy.props.EnumProperty(
        items=[
            ...
            ("2", "Z", "Align to the Z axis of the World."),
        ],
        name="Axis",
        default="-1",
    )
...
def main(self, context):
    ...
    if sync:
        ...
        selected_faces = {
            f
            for f in bm.faces
            if all([utilities_uv.get_loop_selection(loop, uv_layers) for loop in f.loops]) and f.select
        }
            ...
            if len(pre_calc_faces) == 1:
                ...
                for edge in island_edges:
                    if (
                        len(
                            {
                                loop[uv_layers].uv.to_tuple(precision)
                ...
                if not calc_faces:
                    self.report(
                        {"ERROR_INVALID_INPUT"}, "Invalid selection in an island: no faces formed by unique edges."
                    )
                    continue
    ...
    if avg_angle:
        ...
        for face in faces:
            for loop in face.loops:
                ...
                loop[uv_layers].uv = (
                    vec_origin.x + matrix[0][0] * uvs0.x + matrix[0][1] * uvs0.y,
                    vec_origin.y + matrix[1][0] * uvs0.x + matrix[1][1] * uvs0.y,
                )
