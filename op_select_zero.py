...
class op(bpy.types.Operator):
    ...
    bl_description = (
        "Detect degenerate UVs (zero area UV triangles) across all polygons (even hidden) of the selected objects"
    )
...
def select_zero(self):
    ...
    self.report(
        {"WARNING"},
        f"Detected {counter} degenerate UV triangles (THE AFFECTED MESH POLYGONS MAY BE HIDDEN OR UNSELECTED!)",
    )
    return {"FINISHED"}
