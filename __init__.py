...
class Panel_Preferences(AddonPreferences):
    ...
    swizzle_y_coordinate: EnumProperty(
        items=(
    ...
    bake_32bit_float: EnumProperty(
        items=(("8", "8 Bit", ""), ("32", "32 Bit", "")), description="", name="Image depth", default="8"
    )
    ...
    bool_bake_back_color: EnumProperty(
        items=[
    ...
    bake_color_space_def: EnumProperty(
        items=[
    ...
    bool_color_id_vertex_color_gamma: BoolProperty(
        name="Apply gamma to ID Colors in Vextex Color mode for visual consistency", default=False
    )
    bool_modifier_auto_high: BoolProperty(
        name="Detect Objects with Subdiv or Bevel Mods as a Highpoly pair for baking", default=True
    )
    ...
    def draw(self, context):
        ...
        if not hasattr(bpy.types, "ShaderNodeBevel"):
            ...
            col.operator(
                "wm.url_open", text="Get Blender with Bevel Shader", icon="BLENDER"
        ...
        col = box.column(align=True)
        col.operator(
            "wm.url_open", text="Donate", icon="HELP"
        ).url = "https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=ZC9X4LE7CPQN6"
        col.operator(
            "wm.url_open", text="GIT Code", icon="WORDWRAP_ON"
        ).url = "https://github.com/SavMartin/TexTools-Blender"

        col.label(text="Discussions")
        row = col.row(align=True)
        row.operator(
            "wm.url_open", text="BlenderArtists", icon="BLENDER"
        ).url = "https://blenderartists.org/forum/showthread.php?443182-TexTools-for-Blender"
        row.operator(
            "wm.url_open", text="Polycount"
        ).url = "http://polycount.com/discussion/197226/textools-for-blender"
...
class TexToolsSettings(PropertyGroup):
    ...
    size: IntVectorProperty(
        name="Size", size=2, description="Texture & UV size in pixels", default=(512, 512), subtype="XYZ"
    )
    size_dropdown: EnumProperty(
        items=utilities_ui.size_textures, name="Texture Size", update=on_dropdown_size, default="512"
    )
    uv_channel: EnumProperty(items=get_dropdown_uv_values, name="UV", update=on_dropdown_uv_channel)
    UDIMs_source: EnumProperty(
        items=[
    ...
    padding: IntProperty(name="Padding", description="Padding size in pixels", default=4, min=0, max=256)
    ...
    bake_curvature_size: IntProperty(
        name="Curvature", description="Curvature offset in pixels to process", default=1, min=1, max=64
    )
    bake_wireframe_size: FloatProperty(
        name="Thickness", description="Wireframe thickness in pixels", default=1, min=0.1, max=4.0
    )
    bake_bevel_size: FloatProperty(name="Radius", description="Bevel radius", default=0.05, min=0.0, max=16)
    ...
    bake_thickness_distance: FloatProperty(name="Distance", description="AO distance", default=1.0, min=0.0, max=16.0)
    bake_thickness_contrast: FloatProperty(name="Contrast", description="AO contrast", default=1.0, min=0.0, max=2.0)
    bake_thickness_local: BoolProperty(
        name="Only Local", description="Only detect occlusion from the object itself, and not others", default=True
    )
    ...
    bake_freeze_selection: BoolProperty(
        name="Lock", description="Lock baking sets, don't change with selection", default=False
    )
    ...
    texel_get_mode: EnumProperty(
        items=[
            ("IMAGE", "Image", "Per object, get the resolution of the first image found in any used material"),
            ("SIZE", "TexTools Size", "Use the Size specified under the TexTools tab"),
        ]
        + utilities_ui.size_textures,
        name="Texture Size",
        default="IMAGE",
    )
    texel_set_mode: EnumProperty(
        items=[
    ...
    texel_density: FloatProperty(name="Texel", description="Texel size or Pixels per unit ratio", default=256, min=0.0)
    ...
    color_ID_count: IntProperty(
        name="Count", description="Number of color IDs", default=5, update=on_color_count_changed, min=2, max=20
    )
...
class UI_PT_Panel_Units(Panel):
    ...
    def draw(self, context):
            ...
            if obj.type == "MESH":
                ...
                if not obj.data.uv_layers:
                    ...
                    if image:
                        ...
                        col = row.column()
                        col.template_list(
                            "IMAGE_UL_udim_tiles", "", image, "tiles", image.tiles, "active_index", rows=3, maxrows=3
                        )

        col = layout.column(align=True)
        row = col.row(align=True)
        row.operator(
            op_texture_reload_all.op.bl_idname, text="Reload Textures", icon_value=icon_get("op_texture_reload_all")
        )
        ...
        if settings.bversion >= 3.0:
            ...
            row.scale_y = 1.75
            row.operator(
                op_texel_checker_map.op.bl_idname, text="Checker Map", icon_value=icon_get("op_texel_checker_map")
            )
...
class UI_PT_Panel_Layout(Panel):
    ...
    def draw_header(self, _):
        ...
        if bpy.context.preferences.addons[__package__].preferences.bool_help:
            ...
            ).url = "http://renderhjs.net/textools/blender/index.html#uvlayout"
    ...
    def draw(self, context):
        ...
        row = col.row(align=True)
        row.operator(
            op_island_align_world.op.bl_idname, text="Align World", icon_value=icon_get("op_island_align_world")
        )
        ...
        col.operator(
            op_align.op.bl_idname, text="↙", icon_value=icon_get("op_align_bottomleft")
        ...
        col.operator(
            op_align.op.bl_idname, text="↘", icon_value=icon_get("op_align_bottomright")
        ...
        row = col.row(align=True)
        row.operator(
            op_align.op.bl_idname, text="—", icon_value=icon_get("op_align_horizontal")
        ...
        row = col_tr.row(align=True)
        row.operator(
            op_island_rotate_90.op.bl_idname, text="90° CCW", icon_value=icon_get("op_island_rotate_90_left")
        ).angle = -math.pi / 2
        row.operator(
            op_island_rotate_90.op.bl_idname, text="90° CW", icon_value=icon_get("op_island_rotate_90_right")
        ).angle = math.pi / 2
        row = col_tr.row(align=True)
        row.operator(
            op_island_mirror.op.bl_idname, text="Mirror H", icon_value=icon_get("op_island_mirror_H")
        ).is_vertical = False
        row.operator(
            op_island_mirror.op.bl_idname, text="Mirror V", icon_value=icon_get("op_island_mirror_V")
        ).is_vertical = True

        col = box.column(align=True)
        row = col.row(align=True)
        op = row.operator(
            op_island_align_sort.op.bl_idname, text="Sort H", icon_value=icon_get("op_island_align_sort_h")
        )
        op.is_vertical = False
        op.padding = utilities_ui.get_padding()

        op = row.operator(
            op_island_align_sort.op.bl_idname, text="Sort V", icon_value=icon_get("op_island_align_sort_v")
        )
        op.is_vertical = True
        op.padding = utilities_ui.get_padding()
        ...
        row = col.row(align=True)
        row.operator(
            op_island_straighten_edge_loops.op.bl_idname,
            text="Straight",
        ...
        row = col.row(align=True)
        row.operator(
            op_select_islands_identical.op.bl_idname, text="Similar", icon_value=icon_get("op_select_islands_identical")
        )
        row.operator(
            op_select_islands_overlap.op.bl_idname, text="Overlap", icon_value=icon_get("op_select_islands_overlap")
        )

        row = col.row(align=True)
        row.operator(op_select_zero.op.bl_idname, text="Zero", icon_value=icon_get("op_select_zero"))
        row.operator(
            op_select_islands_flipped.op.bl_idname, text="Flipped", icon_value=icon_get("op_select_islands_flipped")
        )

        if settings.bversion >= 3.2:
            row = col.row(align=True)
            row.operator(
                op_select_islands_outline.op.bl_idname, text="Bounds", icon_value=icon_get("op_select_islands_outline")
            )
...
class UI_PT_Panel_Bake(Panel):
    ...
    def draw_header(self, _):
        ...
        if bpy.context.preferences.addons[__package__].preferences.bool_help:
            row.operator(
                "wm.url_open", text="", icon="INFO"
            ).url = "http://renderhjs.net/textools/blender/index.html#texturebaking"
    ...
    def draw(self, context):
        ...
        if settings.bversion >= 3.0:
            ...
            row.scale_y = 1.5
            row.operator(
                op_texture_preview.op.bl_idname, text="Preview Texture", icon_value=icon_get("op_texture_preview")
            )
        ...
        if len(images) > 0:
            ...
            for image in images:
                ...
                if image.filepath != "":
                    row.operator(
                        op_texture_open.op.bl_idname, text="", icon_value=icon_get("op_texture_open")
                    ).name = image.name
                else:
                    if bpy.app.debug_value != 0:
                        row.operator(
                            op_texture_save.op.bl_idname, text="", icon_value=icon_get("op_texture_save")
                        ).name = image.name
                    else:
                        pass
        ...
        col.template_icon_view(
            bpy.context.scene, "TT_bake_mode", scale=5.0, scale_popup=preferences.bake_mode_panel_scale
        )
        ...
        if len(settings.sets) > 0:
            ...
            if count_types["float"] > 0:
                row.operator(
                    "uv.op_select_bake_type", text="", icon_value=icon_get("bake_obj_float")
                ).select_type = "float"

            if count_types["cage"] > 0:
                row.operator(
                    "uv.op_select_bake_type", text="", icon_value=icon_get("bake_obj_cage")
                ).select_type = "cage"
            ...
            countTypes = (
                (0 if count_types["low"] == 0 else 1)
                + (0 if count_types["high"] == 0 else 1)
                + (0 if count_types["cage"] == 0 else 1)
                + (0 if count_types["float"] == 0 else 1)
            )
            if countTypes > 2:
        ...
        col = box.column(align=True)
        col.operator(
            op_bake_organize_names.op.bl_idname, text=f"Organize {len(bpy.context.selected_objects)}x", icon="BOOKMARKS"
        )
...
class UI_MT_op_color_dropdown_convert_from(Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(
            op_color_from_elements.op.bl_idname, text="Mesh Elements", icon_value=icon_get("op_color_from_elements")
        )
        layout.operator(
            op_color_from_materials.op.bl_idname, text="Materials", icon_value=icon_get("op_color_from_materials")
        )
        layout.operator(
            op_color_from_directions.op.bl_idname, text="Directions", icon_value=icon_get("op_color_from_directions")
        )
...
class UI_MT_op_color_dropdown_convert_to(Menu):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(
            op_color_convert_texture.op.bl_idname, text="Texture Atlas", icon_value=icon_get("op_color_convert_texture")
        )
        layout.operator(
            op_color_convert_vertex_colors.op.bl_idname,
            text="Vertex Colors",
...
class UI_PT_Panel_Colors(Panel):
    ...
    def draw_header(self, _):
        ...
        if bpy.context.preferences.addons[__package__].preferences.bool_help:
            row.operator(
                "wm.url_open", text="", icon="INFO"
            ).url = "http://renderhjs.net/textools/blender/index.html#colorid"
    ...
    def draw(self, context):
                    ...
                    if bpy.context.active_object in bpy.context.selected_objects:
                        if len(bpy.context.selected_objects) == 1:
                            if (
                                ...
                                and context.scene.texToolsSettings.color_assign_mode == "MATERIALS"
                            ):
        ...
        row.menu(
            UI_MT_op_color_dropdown_convert_to.bl_idname,
        )  # icon='EXPORT'
...
class UI_PT_Panel_MeshTexture(Panel):
    ...
    def draw_header(self, _):
        ...
        if bpy.context.preferences.addons[__package__].preferences.bool_help:
            row.operator(
                "wm.url_open", text="", icon="INFO"
            ).url = "http://renderhjs.net/textools/blender/index.html#meshtexture"
    ...
    def draw(self, context):
        ...
        if settings.bversion >= 3.2:
            ...
            row.scale_y = 1.5
            row.operator(
                op_meshtex_create.op.bl_idname, text="Create UV Mesh", icon_value=icon_get("op_meshtex_create")
            )
            ...
            if op_meshtex_trim_collapse.is_available():
                row.operator(
                    op_meshtex_trim_collapse.op.bl_idname,
                    text="Collapse Trim",
        ...
        row.scale_y = 1.5
        row.operator(
            op_smoothing_uv_islands.op.bl_idname,
            text="Smooth by UV Islands",
...
def menu_IMAGE_uvs(self, context):
    ...
    layout.separator()
    layout.operator(
        op_island_centralize.op.bl_idname, text="Centralize Position", icon_value=icon_get("op_island_centralize")
    )
...
def menu_VIEW3D_MT_mesh_add(self, context):
    self.layout.operator(
        op_meshtex_pattern.op.bl_idname, text="Create Pattern", icon_value=icon_get("op_meshtex_pattern")
    )
...
def register():
    ...
    bpy.types.Object.TT_CM_Scale = bpy.props.FloatProperty(
        name="Tiling", description="Checker Map scale for the Active Object", default=1, min=0.0001, max=1000
    )
