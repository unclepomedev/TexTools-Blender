...
def main(self, context):
    ...
    selection = {
        ...
        if utilities_uv.get_loop_edge_selection(loop, uv_layers)
    }
    ...
    islands, target_islands = utilities_uv.getSelectedUnselectedIslands(
        bm, uv_layers, selected_faces=selectionFaces, target_faces=selectionFacesTargets
    )
    ...
    for island in islands:
        ...
        if remaining_islands:
            ...
            op_select_islands_outline.select_outline(self, context)
            selectionBorder = {
                loop for face in island for loop in face.loops if utilities_uv.get_loop_edge_selection(loop, uv_layers)
            }
            ...
            for grouped_loops in loopsByTarget:
                ...
                bpy.ops.uv.stitch(
                    use_limit=False, snap_islands=True, midpoint_snap=False, clear_seams=True, mode="EDGE"
                )
