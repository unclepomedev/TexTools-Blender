...
def main(me, bm, uv_layers, selFacesMix, faces_loops, return_discarded_faces=False):
    ...
    if not selFaces:
        ...
        # Active face checks
        if targetFace is None or len({loop for loop in targetFace.loops}.intersection(filteredVerts)) != len(
                targetFace.verts) or targetFace.select == False or len(targetFace.verts) != 4:
...
def MakeUvFaceEqualRectangle(vertsDict, leftUp, rightUp, rightDown, leftDown, startv):
    ...
    if startv is None:
        startv = leftUp.uv
    elif AreVertsQuasiEqual(startv, rightUp):
        startv = rightUp.uv
    elif AreVertsQuasiEqual(startv, rightDown):
        startv = rightDown.uv
    elif AreVertsQuasiEqual(startv, leftDown):
        startv = leftDown.uv
    else:
        startv = leftUp.uv
