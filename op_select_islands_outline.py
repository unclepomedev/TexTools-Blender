...
def select_outline(
    self, context, bm=None, uv_layers=None
):  # , linkloops=True added just for stitch to work, may slow down the script
    if bm is None:
    ...
    if sync:
        ...
        selected_loops = {
            l for f in bm.faces for l in f.loops if utilities_uv.get_loop_edge_selection(l, uv_layers) and l.edge.select
        }
