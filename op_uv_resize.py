...
class op(bpy.types.Operator):
    ...
    size_x: bpy.props.IntProperty(name="Width", description="padding size in pixels", default=1024, min=1, max=8192)
    size_y: bpy.props.IntProperty(name="Height", description="padding size in pixels", default=1024, min=1, max=8192)
    dropdown_size_x: bpy.props.EnumProperty(
        items=utilities_ui.size_textures, name="", update=on_dropdown_size_x, default="1024"
    )
    dropdown_size_y: bpy.props.EnumProperty(
        items=utilities_ui.size_textures, name="", update=on_dropdown_size_y, default="1024"
    )
    ...
    def draw(self, context):
        ...
        layout.label(text="{} to {}".format(size_A, size_B))
    ...
    def execute(self, context):
        ...
        size_A = Vector([bpy.context.scene.texToolsSettings.size[0], bpy.context.scene.texToolsSettings.size[1]])
        size_B = Vector([self.size_x, self.size_y])

        resize_uv(self, context, self.direction, size_A, size_B)
        resize_image(context, self.direction, size_A, size_B)
