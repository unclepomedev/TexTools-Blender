...
class op(bpy.types.Operator):
    ...
    round_mode: bpy.props.EnumProperty(
        name="Round Mode", default="OFF", items=(("OFF", "Off", ""), ("INT", "Int", ""), ("STEPS", "Steps", ""))
    )
    steps: bpy.props.FloatVectorProperty(
        name="Steps",
        description="Incorrectly works with Within Image Bounds",
        default=(0, 0),
        min=0,
        max=10,
        soft_min=0,
    ...
    strength: bpy.props.FloatVectorProperty(
        name="Strength", default=(1, 1), min=-10, max=10, soft_min=0, soft_max=1, size=2, subtype="XYZ"
    )
    rotation: bpy.props.FloatProperty(
        ...
        soft_max=math.pi * 2,
        subtype="ANGLE",
        update=lambda self, _: setattr(self, "rotation_steps", self.rotation)
        if self.rotation < self.rotation_steps
        else None,
    )
    rotation_steps: bpy.props.FloatProperty(
        ...
        max=math.pi,
        subtype="ANGLE",
        update=lambda self, _: setattr(self, "rotation", self.rotation_steps)
        if self.rotation < self.rotation_steps
        else None,
    )
    ...
    bool_bounds_scaling: bpy.props.BoolProperty(
        name="Scale Within Image Bounds",
        default=False,
        description="Scale islands within the 0-1 UV domain when necessary",
    )
