...
def main(self, axis):
    ...
    for obj in selected_obj:
        ...
        # selection_store
        sel_states = [
            (utilities_uv.get_loop_selection(loop, uv_layer), utilities_uv.get_loop_edge_selection(loop, uv_layer))
        ...
        if full_islands:
            for island in full_islands:
                ...
                orient_uvs.append(
                    (x_min, x_max, y_min, y_max, x_min.uv.copy(), x_max.uv.copy(), y_min.uv.copy(), y_max.uv.copy())
                )
        ...
        for bbox, island in zip(orient_uvs, full_islands):
            ...
            # case: when island small
                ...
                if count_valid_bbox:
                    ...
                    self.report(
                        {"WARNING"},
                        f"UV Island with a small area ({island_bbox.area}) was found and scaled "
                        f"to the average size ({scale}) of the other islands. To validate the unwrap, try again.",
                    )
                else:
                    scale = 0.2 / length
                    self.report(
                        {"WARNING"}, f"UV Island with small area ({island_bbox.area}) was detected and scaled to 0.2"
                    )
                utilities_uv.scale_island(island, uv_layer, Vector((scale, scale)), pivot)
            else:
