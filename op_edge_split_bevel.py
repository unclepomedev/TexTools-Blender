...
class op(bpy.types.Operator):
    ...
    radius: bpy.props.FloatProperty(name="Space", description="Space for split bevel", default=0.015, min=0, max=0.35)
...
def slide_uvs(vert, edge, face, edges, vert_rails, vert_to_uv):
    ...
    for v in verts:
        ...
        if IS_DEBUG():
            print(
                "    #{}  rails = {}".format(
                    v.index, [("{} - {}".format(e.verts[0].index, e.verts[1].index)) for e in rails]
                )
            )
...
def get_edge_prev_next(edge, edges):
    ...
    A_extends = [
        ...
        for v2 in e.verts
        if e != edge and e in edges and v2 not in edge.verts and v1 != A
    ]
    B_extends = [
        v2
        for v1 in edge.verts
        for e in v1.link_edges
        for v2 in e.verts
        if e != edge and e in edges and v2 not in edge.verts and v1 != B
    ]
