...
def set_bake_color_space_int(bake_mode):
    ...
    if "normal_" in bake_mode:
        ...
        if (
            (op_bake.modes[bake_mode].material != "")
            or (bake_mode == "transmission" and not prefs().bool_clean_transmission)
            or bake_mode in {"diffuse", "base_color", "sss_color", "emission", "environment", "combined"}
        ):
...
def register():
    ...
    bpy.types.Scene.TT_bake_mode = bpy.props.EnumProperty(
        items=generate_bake_mode_previews(), update=on_bakemode_set, default="normal_tangent.bip"
    )
