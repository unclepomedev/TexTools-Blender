...
def sort_objects(self):
    ...
    if 0 in (len(objects_low), len(objects_high)):
        self.report(
            {"ERROR_INVALID_INPUT"}, f"There are no {'low' if len(objects_low) == 0 else 'high'} poly objects selected"
        )
...
def get_bbox(obj):
    ...
    return {"min": box_min, "max": box_max, "size": (box_max - box_min), "center": box_min + (box_max - box_min) / 2}
