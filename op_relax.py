...
class op(bpy.types.Operator):
    ...
    iterations: bpy.props.IntProperty(
        ...
        soft_max=4,
        default=1,
        description="Repeat Smooth the specified number of times.",
    )
    area_preservation: bpy.props.FloatProperty(
        name="Area Preservation",
        min=0.0,
        max=1.0,
        default=0.95,
        description="Factor of rectification of the area shrink caused by the Smooth operator.",
    )
...
def relax(self, context):
    ...
    bm, uv_layers, faces_by_island = op_meshtex_create.create_uv_mesh(
        self, context, obj, sk_create=False, bool_scale=False, delete_unselected=False, restore_selected=True
    )
    if bm == {"CANCELLED"}:
        return
