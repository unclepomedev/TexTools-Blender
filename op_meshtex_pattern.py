...
class op(bpy.types.Operator):
    ...
    size: bpy.props.IntProperty(name="Size", description="Size X and Y of the repetition", default=4, min=1, max=128)

    scale: bpy.props.FloatProperty(name="Scale", description="Scale of the mesh pattern", default=1, min=0)
    ...
    if mode == "hexagon":
        ...
        if context_override:
            if settings.bversion >= 3.2:
                ...
                bpy.ops.transform.translate(
                    context_override, value=(0, scale * 0.5, 0), constraint_axis=(False, True, False)
                )
        else:
            bpy.ops.transform.translate(value=(0, scale * 0.5, 0), constraint_axis=(False, True, False))
            ...
            if settings.bversion >= 3.2:
                with bpy.context.temp_override(**context_override):
                    bpy.ops.transform.resize(
                        value=(1, 0.5, 1), constraint_axis=(True, True, False), orient_type="GLOBAL"
                    )
            else:
                bpy.ops.transform.resize(
                    context_override, value=(1, 0.5, 1), constraint_axis=(True, True, False), orient_type="GLOBAL"
                )
        else:
        ...
        if context_override:
            if settings.bversion >= 3.2:
                with bpy.context.temp_override(**context_override):
                    bpy.ops.transform.resize(
                        value=(0.5, size / 2, 1), constraint_axis=(True, True, False), orient_type="GLOBAL"
                    )
                    bpy.ops.transform.resize(
                        value=(scale, scale, 1), constraint_axis=(True, True, False), orient_type="GLOBAL"
                    )
                ...
                bpy.ops.transform.resize(
                    context_override, value=(scale, scale, 1), constraint_axis=(True, True, False), orient_type="GLOBAL"
                )
                bpy.ops.transform.translate(
                    context_override, value=(0, (-size / 2) * scale, 0), constraint_axis=(False, True, False)
                )
        else:
            bpy.ops.transform.resize(
                value=(0.5, size / 2, 1), constraint_axis=(True, True, False), orient_type="GLOBAL"
